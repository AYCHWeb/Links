...
const processSpecURL = (specURL, seconds) => {
  ...
  if (respecSpecs.includes(requestURL)) {
    requestURL =
...
const processSVG2files = baseURL => {
  [
    ...
    'text.html',
    'types.html',
  ].forEach(filename => processSpecURL(baseURL + filename, 0));
};
...
const processSpecMap = (key, data) => {
  if (data && data.length > 0) {
    ...
    if ('html.json' === data) {
      processSpecURL('https://html.spec.whatwg.org/', 0);
      return data;
    }
